...
// Package infra 提供基础设施相关的功能实现
// 本文件实现了多智能体系统中的日志回调处理器，支持SSE流式响应和实时数据推送
//
// 主要功能:
// 1. LoggerCallback: 实现eino框架的回调接口，用于处理智能体执行过程中的各种事件
// 2. SSE流式推送: 通过Server-Sent Events向客户端实时推送智能体的执行状态和结果
// 3. 消息处理: 支持普通消息、工具调用、工具结果等多种消息类型的处理和转换
// 4. 异步数据流: 通过goroutine和channel实现非阻塞的数据流处理
//
// 使用场景:
// - 多智能体对话系统的实时响应
// - 工具调用过程的状态监控
...
import (
	...
	"github.com/hildam/deer-flow-go/biz/util"
)

// LoggerCallback 日志回调处理器，用于多智能体系统中的流式响应和日志记录
// 实现了 eino 框架的回调接口，支持 SSE（Server-Sent Events）流式推送和通道输出
...
//
// 参数:
//   - ctx: 上下文对象，用于日志记录和错误处理
//   - event: SSE事件类型，如"message_chunk"、"tool_call_result"等
//   - data: 待推送的聊天响应数据
//
// 返回值:
//   - error: 序列化或推送过程中的错误
